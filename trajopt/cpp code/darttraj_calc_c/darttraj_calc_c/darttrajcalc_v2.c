...
double myfunc(unsigned n, const double* x, double* grad, void* my_func_data)
{
    ...
    target_t.x = dartpath[gridsize - 1].x - (target_init.x + target_init.dx * T);
    target_t.y = dartpath[gridsize - 1].y - (target_init.y + target_init.dy * T);
    target_t.z = dartpath[gridsize - 1].z - (target_init.z + target_init.dz * T);

    double obj_output_val = sqrt(target_t.y * target_t.y + target_t.z * target_t.z);    //minimize 
...
traj_calc_output traj_calc(traj target, double v_init, double rho, double c_d, double A, double m)
{
    ...
    if (result > 0)
    {
        ...
        double dart_hit_phi = round(x[0] * 180 / PI * 10.0) / 10.0 * PI / 180;
        double dart_hit_theta = round(x[1] * 180 / PI * 10.0) / 10.0 * PI / 180;

        traj target_hit[gridsize];
        target_hit[0].x = 0;
        target_hit[0].y = 0;
        target_hit[0].z = 0;
        target_hit[0].dx = v_init * cos(dart_hit_phi) * cos(dart_hit_theta);
        target_hit[0].dy = v_init * sin(dart_hit_phi) * cos(dart_hit_theta);
        target_hit[0].dz = v_init * sin(dart_hit_theta);
        ...
        {
            outputdata.phi = dart_hit_phi;
            outputdata.theta = dart_hit_theta;
