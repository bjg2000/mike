    ...
    try:
        ...
        while True:
            ...
            if distance is not None:
                rolling_distances.append(distance)
                if len(rolling_distances) > 3:
                    rolling_distances.pop(0)

                if len(rolling_distances) == 3:
                    avg_distance = sum(rolling_distances) / 3
                    time_diff = current_time - previous_time
                    speed = (rolling_distances[-1] - rolling_distances[0]) / time_diff
                    print(f"Distance: {avg_distance:.3f} meters, Speed: {speed:.3f} m/s")
                else:
                    print(f"Distance: {distance:.3f} meters, Speed: Calculating...")
